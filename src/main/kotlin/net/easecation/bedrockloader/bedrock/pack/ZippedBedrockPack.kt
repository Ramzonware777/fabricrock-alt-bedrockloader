package net.easecation.bedrockloader.bedrock.pack

import com.google.gson.reflect.TypeToken
import net.easecation.bedrockloader.util.GsonUtil
import java.io.File
import java.io.InputStreamReader
import java.util.zip.ZipFile

class ZippedBedrockPack(file: File) : AbstractBedrockPack() {
    private var pathPrefix: String = ""

    fun getPathPrefix(): String = pathPrefix

    init {
        if (!file.exists()) {
            throw IllegalArgumentException("File not found: " + file.name)
        }

        ZipFile(file).use { zip ->
            val entry = findManifestEntry(zip)
            if (entry == null) {
                throw IllegalArgumentException("No manifest found in " + file.name)
            }
            pathPrefix = entry.name.substringBeforeLast("/", "")
                .let { if (it.isEmpty()) "" else "$it/" }
            val type = object : TypeToken<PackManifest>() {}.type
            manifest = GsonUtil.GSON.fromJson(InputStreamReader(zip.getInputStream(entry)), type)
        }

        manifest?.let {
            if (!it.isValid()) {
                throw IllegalArgumentException("Invalid manifest in " + file.name)
            }

            id = it.header?.uuid.toString()
            version = it.header?.version.toString()
            type = it.modules.stream()
                    .findFirst()
                    .map { module -> module.type }
                    .orElse("resources")
        }
    }

    private fun findManifestEntry(zip: ZipFile): java.util.zip.ZipEntry? {
        // Fast path: manifest at root
        zip.getEntry("manifest.json")?.let { return it }
        zip.getEntry("pack_manifest.json")?.let { return it }

        // Fallback: manifest inside a nested folder (common in some third-party packs)
        return zip.entries().asSequence()
            .filter { !it.isDirectory }
            .filter {
                it.name.endsWith("/manifest.json", ignoreCase = true) ||
                it.name.endsWith("/pack_manifest.json", ignoreCase = true)
            }
            .sortedWith(compareBy(
                { it.name.count { ch -> ch == '/' } },
                { it.name.length }
            ))
            .firstOrNull()
    }
}
