...
object BedrockAddonsLoader {
            ...
            try {
                ...
                when (structureType) {
                    ...
                    PackStructureType.ADDON_DIRECTORY -> {
                        ...
                        if (mcaddonFile != null) {
                            loadMcAddon(mcaddonFile, addonName = file.name, cacheDir = cacheDir)
                        }
                    }
                    PackStructureType.MCADDON_FILE -> {
                        // .mcaddon文件
                        loadMcAddon(file, addonName = file.nameWithoutExtension, cacheDir = cacheDir)
                    }
     ...
     * 加载.mcaddon文件中的所有包
     */
    private fun loadMcAddon(mcaddonFile: File, addonName: String, cacheDir: File) {
        BedrockLoader.logger.info("加载Addon: $addonName (${mcaddonFile.name})")

        val entries = AddonScanner.loadMcAddon(mcaddonFile)
        if (entries.isEmpty()) {
            val embeddedLoaded = loadEmbeddedPackArchivesFromMcAddon(mcaddonFile, addonName, cacheDir)
            if (embeddedLoaded > 0) {
                BedrockLoader.logger.info("  Loaded ${embeddedLoaded} nested packs via fallback")
                return
            }
     ...
     * 从.mcaddon中加载单个子包
     */
    private fun loadEmbeddedPackArchivesFromMcAddon(
        mcaddonFile: File,
        addonName: String,
        cacheDir: File
    ): Int {
        var loadedCount = 0
        val embeddedCacheDir = File(cacheDir, "embedded").apply { mkdirs() }

        ZipFile(mcaddonFile).use { zip ->
            val embeddedEntries = zip.entries().asSequence()
                .filter { entry ->
                    val name = entry.name
                    !entry.isDirectory &&
                    !name.startsWith("__MACOSX/") &&
                    (name.endsWith(".mcpack", ignoreCase = true) || name.endsWith(".zip", ignoreCase = true))
                }
                .toList()
            ...
            BedrockLoader.logger.info("  Fallback: found ${embeddedEntries.size} embedded pack files in addon")

            for ((index, embeddedEntry) in embeddedEntries.withIndex()) {
                try {
                    val rawName = embeddedEntry.name.substringAfterLast("/").ifBlank { "pack_$index.mcpack" }
                    val safeName = rawName.replace(Regex("[^A-Za-z0-9._-]"), "_")
                    val extractedFile = File(embeddedCacheDir, "${mcaddonFile.nameWithoutExtension}_${index}_$safeName")

                    zip.getInputStream(embeddedEntry).use { input ->
                        FileOutputStream(extractedFile).use { output ->
                            input.copyTo(output)
                        }
                    }

                    loadSinglePack(extractedFile, isDirectoryPack = false, addonName = addonName)
                    loadedCount++
                    BedrockLoader.logger.info("  - Loaded embedded pack: ${embeddedEntry.name}")
                } catch (e: Exception) {
                    LoadingErrorCollector.addError(
                        source = "$addonName/${embeddedEntry.name}",
                        phase = LoadingError.Phase.PACK_LOAD,
                        message = "Failed to load embedded pack: ${e.message}",
                        exception = e
                    )
                }
            }
    ...
    private fun loadSinglePack(zipFile: File, isDirectoryPack: Boolean, addonName: String?) {
        ...
        val pathPrefix = (pack as? ZippedBedrockPack)?.getPathPrefix().orEmpty()
        ...
        if (packType.equals("resources")) {
            ...
            // 为该包创建独立的资源上下文
            val resourceContext = ZipFile(zipFile).use { zip ->
                BedrockResourceDeserializer.deserialize(zip, pathPrefix)
            }
            ...
            // 为该包创建独立的行为上下文
            val behaviorContext = ZipFile(zipFile).use { zip ->
                BedrockBehaviorDeserializer.deserialize(zip, pathPrefix)
            }
